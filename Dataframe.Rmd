...
d <- d %>% group_by(location) %>% mutate(mean_positive = weighted.mean(mean_positive, Percentage/100)) %>% mutate(mean_negative = weighted.mean(mean_negative, Percentage/100)) %>% mutate(Percentage = sum(Percentage)) %>% subset(select = -c(Language)) %>% distinct()
...
Export to csv
```{r loaddata, include=TRUE}
write.csv(d,"dataframe.csv", row.names = FALSE)
```
