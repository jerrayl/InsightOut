...
geographical_path <- here("data", "UNSD_geographical_region.csv")
...
Merge CIA data with UN data to include subregion
```{r addsubregions, include=TRUE}
geographical_region <- read_csv(geographical_path)  %>% select("Sub-region Name", "Country or Area") %>% rename(c("subregion"="Sub-region Name", "country"="Country or Area"))

temp0 <- merge(x= geographical_region, y = lang_countries, by.x = "country", by.y = "Country")
```
...
temp1 <- merge(x= depression_prevalence, y = temp0, by.x = "location", by.y = "country")
```
...
```{r statisticalanalysis, include=TRUE}
m1 <- lmer(mean_positive ~ major_depressive_disorder + (1|subregion), data = d, family = binomial, control=glmerControl(optimizer="bobyqa")) 

# FILL IN REGRESSION MODEL HERE
